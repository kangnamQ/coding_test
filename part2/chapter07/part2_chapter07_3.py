...
예를 들어 높이가 19, 14, 10, 17cm 인 떡이 나란히 있고 절단기 높이를 15cm로 지정하면 자른 뒤 떡의 높이는 15, 14, 10, 15cm 가 될 것이다.
...
while(start <= end):
    ...
    if total < M:  # 떡의 양이 부족한 경우 더 많이 자고
        end = mid -1
    else:   # 떡의 양이 충분하다면 덜 자르기
